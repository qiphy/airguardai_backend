...
from virus_data import VIRUS_DB  # <--- Ensure you created virus_data.py
from model import predict_risk

# Initialize Database
init_db()

# IMPORTANT: Set AQICN_TOKEN in your environment variables on Render
...
def compute_env_multiplier(location: str = "Kuala Lumpur") -> dict:
    ...
    """
    # Use the last 7 days (168h) from stored history
    ...
    # Clamp the multiplier to avoid extreme spikes (0.7x to 1.6x)
    alpha = 0.6
...
def latest():
    now = time.time()
    # Check cache to avoid hitting AQICN API limit
...
def predict(payload: PredictRequest):
    # ---------------------------------------------------------
    # STEP 1: Handle User Input (Virus Name vs Sequence)
    # ---------------------------------------------------------
    user_input = payload.protein_sequence.strip()
    
    # Check if the user typed a known virus name (e.g. "Covid", "Flu")
    if user_input.lower() in VIRUS_DB:
        sequence_to_use = VIRUS_DB[user_input.lower()]
        input_type = "virus_name"
        mapped_name = user_input
    else:
        # Assume it is a raw protein sequence
    ...
    # Model C: Environmental Multiplier (History-based)
    ...
    return {
        ...
        "explanation": (
            "Virus similarity is predicted from the submitted protein sequence (or virus name). "
